...
/** BlueBack.ThreadRequest.Editor
*/
#if(UNITY_EDITOR)
namespace BlueBack.ThreadRequest.Editor
{
	/** UpdatePackage
	*/
	public static class UpdatePackage
	{
		...
		*/
		#if(!DEF_USER_BLUEBACK_THREADREQUEST)
		[UnityEditor.MenuItem("BlueBack/ThreadRequest/UpdatePackage/Develop")]
		#endif
		...
		*/
		#if(!DEF_USER_BLUEBACK_THREADREQUEST)
		[UnityEditor.MenuItem("BlueBack/ThreadRequest/UpdatePackage/Last " + Version.packageversion)]
		#endif
		public static void MenuItem_UpdatePackage_Last()
		{
			string t_version = GetLastReleaseNameFromGitHub();
			if(t_version != null){
				string t_name = "https://github.com/bluebackblue/UpmThreadRequest.git?path=BlueBackThreadRequest/Assets/UPM#" + t_version;
				UnityEditor.PackageManager.Requests.AddRequest t_request = UnityEditor.PackageManager.Client.Add(t_name);
				while(t_request.Status == UnityEditor.PackageManager.StatusCode.InProgress){
					if(UnityEditor.EditorUtility.DisplayCancelableProgressBar(t_name,t_name,1.0f) == true){
						break;
		...
		*/
		...
		{
			string t_url = "https://api.github.com/repos/bluebackblue/UpmThreadRequest/releases/latest";
